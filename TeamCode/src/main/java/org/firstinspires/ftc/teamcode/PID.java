...
public class PID extends OpMode {
    ...
    public void loop() {
       ...
       // double curr_power = fly.getPower();
       // double DESIRED_POWER = DESIRED_RPM / MAX_RPM;
       // if (doThing){
      //      motorPower = ((DESIRED_POWER * 12.0) / curr_voltage) * kV; //this works way better than I thought.
      //  } else {
     //       motorPower = DESIRED_POWER;
      //  }
     //   double MOTOR_RPM = (curr_velocity / TPR) * 60;
        fly.setPower(PID_calc(DESIRED_RPM));
        double curr_power = fly.getPower();
        telemetry.addData("Power: ", curr_power);
        telemetry.update();
    }
